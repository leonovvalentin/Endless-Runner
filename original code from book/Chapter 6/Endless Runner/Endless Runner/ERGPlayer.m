...
-(instancetype)init
{
    self = [super initWithImageNamed:@"character.png"];
    {
        ...
        self.engineEmitter = [NSKeyedUnarchiver unarchiveObjectWithFile:
                              [[NSBundle mainBundle] pathForResource:@"jet" ofType:@"sks"]];
...
- (void) setShielded:(BOOL)shielded
{
    if (shielded) {
        if (![self.shield actionForKey:@"shieldOn"]) {
            ...
            SKAction *musicAction = [SKAction playSoundFileNamed:@"shieldCharged.wav" waitForCompletion:NO];
        ...
        SKAction *musicAction = [SKAction playSoundFileNamed:@"shieldSmashed.wav" waitForCompletion:NO];
        [self runAction:musicAction];
    }
    _shielded = shielded;
}
